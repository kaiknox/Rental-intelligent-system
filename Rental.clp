...
(defclass Property
    ...
    (slot Sun_Time
        (type SYMBOL)
        (allowed-symbols none morning afternoon all_day)
        (create-accessor read-write))
    (slot Terrace
        (type SYMBOL)
        (allowed-symbols yes no)
...
;; Detecta la palabra clave EXIT para finalizar la ejecución de inmediato
(deffunction abort-if-exit (?raw)
    (bind ?txt (lowcase (str-cat ?raw)))
    (if (eq ?txt "exit") then
         (printout t "EXIT detectado. Terminando la ejecucion." crlf)
         (halt)))

(deffunction abort-line-if-exit (?raw-line)
    (if (eq (lowcase (str-cat ?raw-line)) "exit") then
        (printout t "EXIT detectado. Terminando la ejecucion." crlf)
        (halt)))
...
(defrule ask-economics
   ...
   (bind ?question (str-cat"¿Cual es vuestro presupuesto maximo objetivo? (max: "(* ?inc 0.4)"):"))
   (bind ?bud (ask-number ?question))
   (while (> ?bud (* ?inc 0.4)) do
      (printout t "El presupuesto maximo no puede ser mayor al 40% del ingreso mensual. Introdúcelo de nuevo." crlf)
...
(defrule evaluate-basic-criteria
   ...
   (if (eq ?appliances yes)
    ...
    (if (eq ?sun_time none)
      then (bind ?missing (insert$ ?missing (+ (length$ ?missing) 1) sin-sol)))
    (if (and (and (eq ?balcony no) (eq ?garden no)) (eq ?terrace no))
      then (bind ?missing (insert$ ?missing (+ (length$ ?missing) 1) falta-exterior)))
    (if (eq ?noise high)
      then (bind ?missing (insert$ ?missing (+ (length$ ?missing) 1) mucho-ruido)))
    (if (eq ?safe low)
        then (bind ?missing (insert$ ?missing (+ (length$ ?missing) 1) falta-seguridad)))
